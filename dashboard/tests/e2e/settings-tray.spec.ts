 ...
 * settings button is clicked. Covers open/close triggers and field behaviour.
 *
 * The settings button (#button-settings) is only rendered when at least one
 * search tab exists, so these tests use CONFIG_WITH_BOTH + MOCK_SEARCH_EMPTY.
 */

import { test, expect } from "@playwright/test";
import {
  mockAPIs, goto,
  CONFIG_WITH_BOTH, MOCK_SEARCH_EMPTY,
} from "./helpers";
...
test.describe("Settings tray", () => {
  ...
  test("config-loaded values populate tray fields", async ({ page }) => {
    ...
    await mockAPIs(page, { config, searches: [MOCK_SEARCH_EMPTY] });
    await goto(page);
